                     ...
                     char* interface/* lent */)
{
	...
	if (ntohs(ethhdr->ether_type) == ETHERTYPE_ARP)
	{
		...
		printf("Sender ARP Address %s\n", inet_ntoa(temp));
		struct in_addr temp1;
		temp1.s_addr = arphdr->ar_tip;
		printf("Receiver ARP Address %s\n", inet_ntoa(temp1));
	...
	else if (ntohs(ethhdr->ether_type) == ETHERTYPE_IP) 
	{
		...
		if(iphdr->ip_p == 89)
		{
			...
			{
				handlehellopackets(sr,packet);
			}
			else if(ospfhdr->type == OSPF_TYPE_LSU)
			{
				handlelsupackets(sr, packet);
			}
			return;
		}
 ...
 *
 *---------------------------------------------------------------------*/
void sendarprequest(struct sr_instance* sr, uint8_t * packet, unsigned int len, char* interface, uint32_t dstip, int gateway) 
{
	struct ip *iphdr = (struct ip*)(packet + sizeof(struct sr_ethernet_hdr));
	if(iphdr->ip_p != UDP)
	{	
	...
	if(pkt != NULL)
	{
		...
		arphdr->ar_pln = 4;
		if(gateway!=1){
		struct sr_rt* rtentry = routingtablelookup(dstip, sr,0);
		if (rtentry != NULL)
		{	
			dst = rtentry->gw.s_addr;
		}
		arphdr->ar_tip = dst;
		struct sr_if* ipcheck = sr->if_list;
		while(ipcheck)
		{
		if(dst == ipcheck->ip)
		{
		...
		printf("ARP Request Sentdst %s\n", inet_ntoa(temp));}
		else
		{
		arphdr->ar_tip = dstip;
		struct in_addr temp1;
		temp1.s_addr = dstip;
		printf("ARP Request Sentip %s\n", inet_ntoa(temp1));}
		struct in_addr temp2;
		temp2.s_addr = arphdr->ar_tip;
		if(temp2.s_addr == 0)
		{
		arphdr->ar_tip = dstip;
		printf("ARP Request Sent %s\n", inet_ntoa(temp2));}
		int i;
		...
		sr_send_packet(sr, pkt, len_new, interface);
		if(pkt!=NULL){
		free(pkt);}
	}
	else
 ...
 *---------------------------------------------------------------------*/
 void forwardippacket(struct sr_instance* sr, uint8_t *packet, int len, char* interface)
 {
 		...
 		if (iphdr)
 		{
 			struct sr_rt* rtentry = routingtablelookup(iphdr->ip_dst.s_addr, sr,0);
			if (rtentry != NULL) 
			{
				sendtoserver(sr, packet, len, rtentry->interface, rtentry->gw.s_addr);
				return;
			}
			else 
			{
...
void sendtoserver(struct sr_instance* sr, uint8_t* packet, int len, char* interface, uint32_t dst) 
{
	...
	else 
	{
		...
		sendarprequest(sr, packet, len, interface, iphdr->ip_dst.s_addr, 0);
		return;
	}
}
...
void sendtogateway(struct sr_instance *sr, uint8_t *packet, int len) 
{
	...
	char* interface;
	uint32_t gw;
	struct sr_rt *routingtable = gatewaylookup(sr, iphdr->ip_dst.s_addr);
	if(routingtable)
	{
		interface = routingtable->interface;
		gw = routingtable->gw.s_addr;
	}
	if (checkcache(gw) == 1)
	{
		struct arpcache *temp = start;
	...
	{
		addtoqueue(packet, len);
		sendarprequest(sr, packet, len, interface, gw, 1);
		return;
	}
}
...
void sendicmp(struct sr_instance * sr, uint8_t* packet, uint8_t type, uint8_t code)
{
	...
	*p = 0x45;
	...
	struct sr_rt *routingtable = gatewaylookup(sr,iphdr->ip_src.s_addr);
	if(routingtable){
		interface = routingtable->interface;}
	if(interface!=NULL)
	{
		...
		while(ipcheck)
		{
			if(iphdr->ip_dst.s_addr == ipcheck->ip)
			{
				newiphdr->ip_src = iphdr->ip_dst;
			}
			ipcheck = ipcheck->next;
		}
		if(newiphdr->ip_src.s_addr == 0)
		{
	...
	else
	{
		while (cached != NULL) 
		{
			if(cached->packet != NULL)
			{
				...
				{
					struct sr_rt* rtentry = routingtablelookup(cached->ip, sr,0);
					if (rtentry != NULL)
					{	
						sendtoserver(sr, cached->packet, cached->len, rtentry->interface, rtentry->gw.s_addr);
						cached->validator = 0;
					}
					else
					{
